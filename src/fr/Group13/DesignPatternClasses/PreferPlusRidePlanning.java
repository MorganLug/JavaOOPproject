 ...
 * @see RidePlanning
 */
public class PreferPlusRidePlanning implements RidePlanning {
	
	/**
	 * Constructor of the class
	 */
	public PreferPlusRidePlanning() {}
	...
	/**
	 * @param departure coordinates and an array of stations
	 * @return closest online station with at least free slot, or null ; prefering plus stations
	 */
	@Override
	public Station aStation(double aX, double aY, ArrayList<Station> stations) {
		...
		try {
			for (Station station2 : stations) {
				//checks if the station is online and has at least one free slot
				if ((station2.getIsOnline() == true) & (station2.findFreeSlot() != null)) {
					double sX = station2.getCoordinateX();
					double sY = station2.getCoordinateY();
					//checks if this station is closer to the departure coordinates than the closer station amongst the station before this one
			...
			if (station == null) {
				...
				for (Station station2 : stations) {
					//checks if the station is online and has at least one free slot, and is a PlusStation
					if ((station2.getIsOnline() == true) & (station2.findFreeSlot() != null) & (station2 instanceof PlusStation)) {
						double sX = station2.getCoordinateX();
						double sY = station2.getCoordinateY();
						//checks if the Plus Station is closer to point B than 1.1xdistanceOfClosestStation
						if (squareDistance * 1.1 * 1.1 > (aX - sX)*(aX - sX) + (aY - sY)*(aY - sY)) {
							station = station2;
							squareDistance = (aX - sX)*(aX - sX) + (aY - sY)*(aY - sY);
						}
					}
